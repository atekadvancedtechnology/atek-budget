...
export default async function CashflowPage({ params, searchParams }: PageProps) {
  ...
  const responsibles = selectedPeriod
    ? Array.from(
        new Set([
          ...selectedPeriod.incomes.map((income) => income.responsibleName),
          ...selectedPeriod.expenses.map((expense) => expense.responsibleName)
        ])
      )
    : [];
  ...
  const rows = selectedPeriod
    ? [
        ...responsibles.map((name) => {
          const personIncomes = selectedPeriod.incomes.filter((income) => income.responsibleName === name);
          const incomeQ1 = sum(
          ...
          const expenseQ1 = sum(selectedPeriod.expenses.filter((expense) => expense.responsibleName === name).map((expense) => expense.amountQ1));
          const expenseQ2 = sum(selectedPeriod.expenses.filter((expense) => expense.responsibleName === name).map((expense) => expense.amountQ2));
          ...
          return {
            ...
            gastoQ1: expenseQ1,
            ahorroQ1: savingQ1,
            restoQ1: incomeQ1 - expenseQ1 - savingQ1,
            ingresoQ2: incomeQ2,
            gastoQ2: expenseQ2,
            ahorroQ2: savingQ2,
            restoQ2: incomeQ2 - expenseQ2 - savingQ2
          };
        }),
        {
          name: "Familia",
